...
function AdminTransactions() {
  ...
  const handleViewTransactions = (user) => {
    ...
    // Filter transactions by user account number
  ...
  const handleFix = async (id) => {
    try {
      await fetch(`http://127.0.0.1:8080/api/admin/fix/${id}`, {
        method: "PUT"
      });

      fetchData(); // refresh both users and transactions
      
      // Also refresh user transactions view if viewing a user
      if (selectedUser) {
        const filtered = transactions.filter(
          t => t.accountNumber === selectedUser.accountNumber
        );
        setUserTransactions(filtered);
      }
      
      showToast("Transaction fixed ✅", "success");
    } catch (err) {
      showToast("Error fixing transaction", "error");
  ...
  const handleComplete = async (id) => {
    try {
      await fetch(`http://127.0.0.1:8080/api/admin/complete/${id}`, {
        method: "PUT"
      });
          ...
          <div className="p-8">
            ...
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-800">
                {selectedUser ? `Transactions of ${selectedUser.name}` : "Transaction Management"}
              </h1>
              <p className="text-gray-500 mt-2">
                {selectedUser ? "View this user's transaction history" : "View and manage user transactions"}
              </p>
            </div>

            {/* Search Bar - Only show when viewing users list */}
            {!selectedUser && (
              <div className="mb-6">
                <input
                  type="text"
            ...
            {!selectedUser ? (
              <div className="bg-white rounded-xl shadow-md p-6">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-sm text-gray-500">Total: {
                    searchTerm
                      ? users.filter(u => 
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-300">
                      <tr>
                        ...
                        <th className="text-left p-3 font-semibold text-gray-700">User Name</th>
                        ...
                        users
                          ...
                          .map((user) => (
                            <tr key={user.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                              ...
                              <td className="p-3 text-gray-800 font-semibold">{user.name}</td>
                              <td className="p-3 text-gray-600">{user.accountNumber}</td>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {userTransactions.length === 0 ? (
                        <tr>
                          <td colSpan="6" className="p-4 text-center text-gray-500">No transactions found for this user</td>
                        </tr>
                      ) : (
